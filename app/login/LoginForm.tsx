...
<form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
  ...
  <div className="relative">
    <LockKeyholeOpen className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-gray-500" size={20} />
    <input
      {...register("password")}
      type="password"
        ...
        <Select.Portal>
          <Select.Content className="bg-white dark:bg-gray-800 rounded-md shadow-md overflow-hidden">
            <Select.Viewport className="p-1">
              {["Librarian", "Member", "Institution", "System Admin"].map((option) => (
                <Select.Item
                  key={option}
                  value={option}
  ...
  <div className="text-center w-full">
    <Text size="2">
      ...
      <Link className="text-primary underline" href="/auth/librarian/signup">
        Sign up
      </Link>
    </Text>
  </div>
