...
const bookCopySchema = z.object({
    ...
    callNo: z.string().min(3, "Call number must be at least 3 characters").max(20).optional(),
...
const AddBookCopyModal = () => {
    ...
    const { register, handleSubmit, control, formState: { errors, isSubmitting, isValid }, setValue, reset } = useForm<AddBookCopy>({ mode: "onChange", resolver: zodResolver(bookCopySchema) });
    ...
    const handleBookQueryChange = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        <Dialog.Root open={isOpen} onOpenChange={(val) => !val && close()}>
            <Dialog.Portal>
                ...
                <Dialog.Content className="fixed left-1/2 top-1/2 w-[95vw] max-w-2xl max-h-[90vh] overflow-y-auto -translate-x-1/2 -translate-y-1/2 rounded-xl bg-white dark:bg-gray-900 p-8 shadow-lg transition-all duration-300 ease-in-out focus:outline-none">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 max-md:flex max-md:flex-col md:grid-cols-2 gap-6">
                            ...
                            >
                                ...
                                {name === "book" ? (
                                    ...
                                    <input
                                        id={name}
                                        type={type}
                                        {...register(name, { required: !isOptional ? `${label} is required` : false })}
